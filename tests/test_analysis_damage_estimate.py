...
def test_cap_up_breakdown_team_wide_includes_orbs():
    bt = _bucketed(front=(1, 2), cap_orbs=2)
...
def test_cap_up_breakdown_caps_free_orbs_to_one_per_character():
    bt = _bucketed(front=(1,), cap_orbs=3)
    team_wide, self_only = damage_estimate.cap_up_breakdown_for_dps(bt, 1)
    assert team_wide == pytest.approx(100_000.0)
    assert self_only == pytest.approx(0.0)
...
def test_best_skills_builds_label_when_sheet_name_is_missing(conn):
    c, fid = conn
    repo.insert_skills(c, fid, [
        {"slot_order": 1, "name": None, "kind": "active",
         "power_min": 80, "power_max": 80, "hits": 5,
         "description": "5x AoE Sword (5x 80 Power)"},
    ])
    out = damage_estimate._best_skills_for(c, fid, multi_cast=1.0, top=1)
    assert out[0].name == "5x AoE Sword"
